    ...
    getChildren(element?: any): vscode.ProviderResult<any[]> {
            ...
            if (element.children) {
                ...
                if (element && element.itemType !== "Workflow" && element.itemType !== "Server") {
                    ...
                    options = {
                        method: "GET",
                        uri: `https://${serv}:${port}/vco/api/workflows/${element.Path}/content`,
                        headers: {
                        "User-Agent": "Request-Promise",
                        "Authorization": `Basic ${auth}`
                        },
                    ...
                    options = {
                        method: "GET",
                        uri: `https://${serv}:${port}/vco/api/categories?categoryType=WorkflowCategory&isRoot=true`,
                        headers: {
                        "User-Agent": "Request-Promise",
                        "Authorization": `Basic ${auth}`
                        },
                        ...
                        if (result.relations) {
                            ...
                            result.relations.link.forEach(element => {
                                if (element.rel !== "permissions") {
                                    ...
                                    if (element.rel === "down") {
                                        items.push(new WorkflowItem(name, id, _type, null, (_type === "Workflow") ? this.context.asAbsolutePath("dep/layers.svg") : this.context.asAbsolutePath("dep/folder.svg"), vscode.TreeItemCollapsibleState.Collapsed, null, serv, auth, port));
                                    }
                                }
                            });
